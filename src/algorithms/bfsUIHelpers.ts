...
// Validate BFS inputs and build the goal payload used by runBfs.
export const prepareBfsRunInputs = ({
  nodes,
  goalType,
  startNodeLabel,
  goalNodeLabel,
  ...
  if (nodes.length === 0) {
    return { ok: false, error: 'Add nodes before running BFS.' }
  }
...
// Build the final BFS status line after traversal completes.
export const buildBfsCompletionStatus = (result: BfsResult, nodes: GraphNode[]) => {
